...
2. At the top of the page, users find the page title and actions that affect the index as a whole.
3. At the top of the index, users can use filters, sorting, and multi-select actions that affect the list below.
4. In the main body of the index, users find the individual resource objects that they want to view or manage.
...
: Resource objects, such as products, orders and inventory items, are at the heart of enterprise operations. While resource types can be very different, they typically share many general activities, such as adding, finding, or taking action. Use the resource index layout pattern when users need to organize objects and carry out such activities. An example can be found in Products.
...
function IndexFiltersDefault() {
  ...
  const {Badge, Card, ChoiceList, IndexFilters, IndexTable, Page} = window.Polaris;
  const {useIndexResourceState} = window.Polaris;
  ...
  // Mock useSetIndexFiltersMode for this example
